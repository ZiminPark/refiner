...
The AI refinement prompt is now editable from `src/app/app/settings/page.tsx` and persisted via the `usePromptSetting` hook in `src/hooks/usePromptSetting.ts`, which currently wraps `useLocalStorage`. When updating either location, keep the prompt default text in `src/lib/prompt.ts` as the single source of truth and preserve the TODO about migrating persistence from localStorage to the database.
