...
- Add a `--texture-opacity` custom property to control the paper grain strength; expose a `.texture-none` utility for potential future toggles.
...
- Ensure dark mode either switches to current slate scheme or introduces a muted charcoal paper variant without neon accents.
...
## Open Questions
- Should dark mode adopt the beige paper look (warm gray) or keep the current slate scheme for stronger contrast at night?
- Do we want a user toggle to disable the texture for bandwidth or preference reasons?
